...
JavaScript 虽说是一门脚本语言，但是其内部同样包含了很多标准库，基础的变量声明、流程控制、运算符、数组（ Array ）、日期（ Date ）、数学函数（ Math ）等编程语言的特性。当然也是现在的主流的变成语言，能够实现复杂的计算与交互。包含：闭包、匿名（lambda）函数，元编程等特性。
...
当前最新的规范是第 12 版，2021 年 6 月发布的。具体最新的可以通过打开上面的地址， 查看。 
...
当然以上都是基础的笼统的名词概念。细节说起来需要太多太多篇幅了。
